...
int main() {
	...
	EntertainmentCollection<Movie> shelf;
	...
	while (true) {
		...
		if (sel == 1)
		{
			...
			catch (EntertainmentCollection<Movie>::ShelfFull_ e)
			{
				system("CLS");
		...
		else if(sel == 2)
		{
			...
			catch (EntertainmentCollection<Movie>::ShelfEmpty_ e)
			{
				system("CLS");
				std::cout << "Shelf is empty. Cannot take a movie. " << std::endl;
				continue;
			}
			catch (EntertainmentCollection<Movie>::SlotEmpty_ e)
			{
				system("CLS");
